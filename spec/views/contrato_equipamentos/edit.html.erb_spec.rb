...
RSpec.describe "contrato_equipamentos/edit", type: :view do
  before(:each) do
    @contrato_equipamento = assign(:contrato_equipamento, ContratoEquipamento.create!(
      :contrato_id => 1,
      :equipamento_id => 1,
      :quantidade => 1,
      :vl_tabela => 1.5,
  ...
  it "renders the edit contrato_equipamento form" do
    render

    assert_select "form[action=?][method=?]", contrato_equipamento_path(@contrato_equipamento), "post" do
